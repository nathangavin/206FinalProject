...
public class MistakesScreen extends UserGUIElement {
	...
	protected MistakesScreen(JFrame frame, Users users, UserData user) {
		...
		_spellingArea.setPreferredSize(new Dimension(230, 50));
		_spellingArea.addActionListener(this);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().equals(_removeWord)) {
			...
			String speech = "";
			try {
				speech = _mistakes.getWord(_words.getSelectedItem().toString()).getWord();
			} catch (NoSuchWordException e1) {
				e1.printStackTrace();
			}
			speakWord(speech);
			...
			_GUI.revalidate();
			_GUI.repaint();
		} else if (e.getSource().equals(_spellingArea)) {
			String word = _words.getSelectedItem().toString();
			
			String input = _spellingArea.getText().trim();
			...
			if (word.equals(input)) {
				_advice.setText("Correct!");
				_advice.setOpaque(true);
				_advice.setBackground(Color.white);
				_advice.setForeground(Color.black);
			} else {
				_advice.setText("Incorrect");
				_advice.setOpaque(true);
				_advice.setBackground(Color.black);
				_advice.setForeground(Color.white);
			}
		}
