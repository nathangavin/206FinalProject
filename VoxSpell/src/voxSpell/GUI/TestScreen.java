...
public class TestScreen extends UserGUIElement {
	...
	public TestScreen(JFrame frame,Users users, UserData user,Topic topic , WordList list) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource().equals(_speak)) {
			...
			_GUI.getContentPane().add(new StatsScreen(_GUI, _users, _user, this, _topic));
			_GUI.revalidate();
			_GUI.repaint();
			...
			if (input.equals(word)) {
				...
				if (_wordNum+1 == _maxWords) {
					...
					_GUI.getContentPane().add(new CompletionScreen(_GUI,_users, _user, _topic, _score));
